...
import org.altoro.core.services.filter.CorsInterceptor;
...
public class FullNodeHttpApiService implements Service {
  ...
  @Autowired
  private GenerateAddressFileServlet generateAddressFileServlet;
  @Autowired
  private AddressFileListServlet addressFileListServlet;
  @Autowired
  private NewTransactionSignServlet newTransactionSignServlet;
  @Autowired
  private DelegateAccountServlet delegateAccountServlet;
  @Autowired
  private UnDelegateAccountServlet UnDelegateAccountServlet;
  @Autowired
  private DelegateServlet delegateServlet;
  @Autowired
  private TransferPasswordServlet transferPasswordServlet;
  @Autowired
  private ExportAddressFileServlet exportAddressFileServlet;
  @Autowired
  private ImportAddressFileServlet importAddressFileServlet;
  @Autowired
  private DeleteAddressFileServlet deleteAddressFileServlet;
  @Autowired
  private GetAddressTransactionListServlet getAddressTransactionListServlet;
  @Autowired
  private GetNowBlockNumServlet getNowBlockNumServlet;
  @Autowired
  ...
  public void start() {
    try {
      ...
      context.addServlet(new ServletHolder(generateAddressFileServlet), "/generateaddressfile");
      context.addServlet(new ServletHolder(addressFileListServlet), "/addressfilelist");
      context.addServlet(new ServletHolder(newTransactionSignServlet), "/newtransactionsign");
      context.addServlet(new ServletHolder(delegateAccountServlet), "/delegateaccount");
      context.addServlet(new ServletHolder(UnDelegateAccountServlet), "/undelegateaccount");
      context.addServlet(new ServletHolder(delegateServlet), "/delegateList");
      context.addServlet(new ServletHolder(transferPasswordServlet), "/transferpassword");
      context.addServlet(new ServletHolder(exportAddressFileServlet), "/exportaddressfile");
      context.addServlet(new ServletHolder(importAddressFileServlet), "/importaddressfile");
      context.addServlet(new ServletHolder(deleteAddressFileServlet), "/deleteaddressfile");
      context.addServlet(new ServletHolder(getAddressTransactionListServlet), "/getaddresstransactionlist");
      context.addServlet(new ServletHolder(getNowBlockNumServlet), "/getnowblocknum");
      ...
      context.addFilter(new FilterHolder(corsInterceptor), "/*",
