    ...
    while (true) {
      ...
      try {
        JLibsodium.cryptoGenerichashBlake2bInitSaltPersonal(
                new JLibsodiumParam.Blake2bInitSaltPersonalParams(state, null, 0, 64, null,
                        Constant.ZTRON_EXPANDSEED_PERSONALIZATION));
        JLibsodium.cryptoGenerichashBlake2bUpdate(new JLibsodiumParam.Blake2bUpdateParams(state, blob, 34));
        JLibsodium.cryptoGenerichashBlake2bFinal(new JLibsodiumParam.Blake2bFinalParams(state, res, 11));
