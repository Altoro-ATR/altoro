...
import org.altoro.api.GrpcAPI.*;
import org.altoro.api.GrpcAPI.DecryptNotes.NoteTx;
import org.altoro.api.GrpcAPI.Return.response_code;
import org.altoro.api.GrpcAPI.TransactionExtention.Builder;
...
import org.altoro.common.overlay.discover.node.NodeHandler;
...
import org.altoro.core.actuator.Actuator;
import org.altoro.core.actuator.ActuatorFactory;
import org.altoro.core.actuator.VMActuator;
import org.altoro.core.capsule.*;
import org.altoro.core.capsule.BlockCapsule.BlockId;
import org.altoro.core.capsule.utils.MarketUtils;
import org.altoro.core.config.args.Args;
import org.altoro.core.db.BandwidthProcessor;
import org.altoro.core.db.EnergyProcessor;
...
import org.altoro.core.zen.ShieldedTRC20ParametersBuilder;
import org.altoro.core.zen.ShieldedTRC20ParametersBuilder.ShieldedTRC20ParametersType;
import org.altoro.core.zen.ZenTransactionBuilder;
import org.altoro.core.zen.address.*;
import org.altoro.core.zen.note.Note;
import org.altoro.core.zen.note.NoteEncryption;
import org.altoro.core.zen.note.NoteEncryption.Encryption;
import org.altoro.core.zen.note.OutgoingPlaintext;
...
import org.altoro.protos.contract.ShieldContract.*;
import org.altoro.protos.contract.SmartContractOuterClass.CreateSmartContract;
import org.altoro.protos.contract.SmartContractOuterClass.SmartContract;
import org.altoro.protos.contract.SmartContractOuterClass.SmartContractDataWrapper;
...
import static org.altoro.common.utils.Commons.getAssetIssueStoreFinal;
import static org.altoro.common.utils.Commons.getExchangeStoreFinal;
import static org.altoro.common.utils.WalletUtil.isConstant;
import static org.altoro.core.config.Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL;
import static org.altoro.core.config.Parameter.DatabaseConstants.*;
...
public class Wallet {
  ...
  public DiversifierMessage getDiversifier() throws ZksnarkException {
    ...
    byte[] d;
    while (true) {
      d = org.altoro.keystore.Wallet.generateRandomBytes(Constant.ZC_DIVERSIFIER_SIZE);
  ...
  public BytesMessage createSpendAuthSig(SpendAuthSigParameters spendAuthSigParameters)
    ...
    LibrustzcashParam.SpendSigParams spendSigParams = new LibrustzcashParam.SpendSigParams(
  ...
  public BytesMessage createShieldNullifier(NfParameters nfParameters) throws ZksnarkException {
    ...
    LibrustzcashParam.ComputeNfParams computeNfParams = new LibrustzcashParam.ComputeNfParams(
  ...
  public NodeList listNodes() {
    ...
    nodeHandlerMap.entrySet().stream()
            .forEach(v -> {
              org.altoro.common.overlay.discover.node.Node node = v.getValue()
                      .getNode();
      ...
      for (Transaction transaction : block.getTransactionsList()) {
        ...
        for (int index = 0; index < stContract.getReceiveDescriptionList().size(); index++) {
          ...
          if (notePlaintext.isPresent()) {
            ...
            if (!JLibrustzcash
                    .librustzcashIvkToPkd(new LibrustzcashParam.IvkToPkdParams(ivk, noteText.getD().getData(),
                            pkD))) {
              continue;
            }
  ...
  private Optional<DecryptNotesTRC20.NoteTx> getNoteTxFromLogListByIvk(
    ...
    if (!ArrayUtils.isEmpty(logData) && logType > 0 && logType < 4) {
      ...
      if (notePlaintext.isPresent()) {
        ...
        byte[] pkD = new byte[32];
        if (!JLibrustzcash
                .librustzcashIvkToPkd(new LibrustzcashParam.IvkToPkdParams(ivk, noteText.getD().getData(), pkD))) {
   ...
   * todo ******altoro******
   */
  public void addAccountTransaction(byte[] address,ByteString txId){
    dbManager.addAccountTransaction(address,txId);
  }

  public DelegateList getDelegateList() {
    DelegateList.Builder builder = DelegateList.newBuilder();
    List<DelegateCapsule> allDelegate = dbManager.getDelegateStore().getAllDelegate();
    allDelegate.forEach(delegateCapsule -> {
              builder.addDelegates(delegateCapsule.getInstance());
            }
    );
    return builder.build();
  }
  ...
  public GrpcAPI.Return transactionSignAndBroadcastTransaction(TransactionCapsule transaction, SignInterface signInterface){
    TransactionCapsule trx = new TransactionCapsule(transaction.getInstance());
    trx.sign(signInterface.getPrivateKey());
    return broadcastTransaction(trx.getInstance());
  }

  public GrpcAPI.Return transactionSignAndBroadcastTransaction(TransactionCapsule transaction, AccountKeyStore accountKeyStore){
    TransactionCapsule trx = new TransactionCapsule(transaction.getInstance());
    trx.sign(Hex.decode(accountKeyStore.getPriKey()));
    return broadcastTransaction(trx.getInstance());
  }
  ...
  public  List<org.altoro.common.entity.TransactionInfo> getAddressTransactionList(ByteString address) {
    if (Objects.isNull(address)) {
      return null;
    }
    List<org.altoro.common.entity.TransactionInfo> list = new ArrayList<>();
    try {
      AccountTransactionCapsule accountTransactionCapsule = chainBaseManager.getAccountTransactionStore().get(address.toByteArray());
      for (AccountTransaction.AccountTx accountTx : accountTransactionCapsule.getAccountTransactionList()) {
         Map<String,String> map = new HashMap<>();
        TransactionCapsule transactionCapsule = chainBaseManager.getTransactionStore().get(accountTx.getTxid().toByteArray());
        org.altoro.common.entity.TransactionInfo transactionInfo = null;
        if(transactionCapsule != null){
           transactionInfo = dbManager.resolvingTransaction(address,transactionCapsule,true);
        }
